...
static double gettime1900d(void)
{
    struct timeval tv;
...
static double lfp_to_d(l_fixedpt_t lfp)
{
    double ret;
...
static double sfp_to_d(s_fixedpt_t sfp)
{
    double ret;
...
#if ENABLE_FEATURE_NTPD_SERVER
static l_fixedpt_t d_to_lfp(double d)
{
    l_fixedpt_t lfp;
...
static s_fixedpt_t d_to_sfp(double d)
{
    s_fixedpt_t sfp;
...
static double dispersion(const datapoint_t *dp)
{
    return dp->d_dispersion + FREQ_TOLERANCE * (G.cur_time - dp->d_recv_time);
}

static double root_distance(peer_t *p)
{
...
static void set_next(peer_t *p, unsigned t)
{
 ...
 */
static void filter_datapoints(peer_t *p)
{
    int i, idx;
    double sum, wavg;
    datapoint_t *fdp;
    ...
    for (i = 0; i < NUM_DATAPOINTS; i++)
    {
        ...
        else if (maxoff == fdp[idx].d_offset)
        {
            maxoff += 1;
        }
        else
...
static void reset_peer_stats(peer_t *p, double offset)
{
    int i;
...
static len_and_sockaddr *resolve_peer_hostname(peer_t *p)
{
...
#define add_peers(s, key_entry) \
    add_peers(s)
#endif
static void add_peers(const char *s, key_entry_t *key_entry)
{
    llist_t *item;
    peer_t *p;
...
#if ENABLE_FEATURE_NTP_AUTH
static void hash(key_entry_t *key_entry, const msg_t *msg, uint8_t *output)
{
    union
    {
        md5_ctx_t m;
...
static void send_query_to_peer(peer_t *p)
{
    if (!p->p_lsa)
        return;
...
static NOINLINE void step_time(double offset)
{
    llist_t *item;
    double dtime;
...
static int compare_point_edge(const void *aa, const void *bb)
{
    const point_t *a = aa;
    const point_t *b = bb;
    if (a->edge < b->edge)
...
static int compare_survivor_metric(const void *aa, const void *bb)
{
...
static int fit(peer_t *p, double rd)
{
...
static peer_t *select_and_cluster(void)
{
    peer_t     *p;
    llist_t    *item;
    int        i, j;
 ...
 */
static void set_new_values(int disc_state, double offset, double recv_time)
{
...
static NOINLINE int update_local_clock(peer_t *p)
{
    int rc;
    struct timex tmx;
 ...
 * (helpers first)
 */
static unsigned poll_interval(int upper_bound)
{
...
static void adjust_poll(int count)
{
...
static NOINLINE void recv_and_process_peer_pkt(peer_t *p)
{
    int         rc;
    ssize_t     size;
    msg_t       msg;
...
#if ENABLE_FEATURE_NTPD_SERVER
static NOINLINE void recv_and_process_client_pkt(void /*int fd*/)
{
    ssize_t          size;
    //uint8_t          version;
    len_and_sockaddr *to;
 ...
 */
#if ENABLE_FEATURE_NTP_AUTH
static key_entry_t *find_key_entry(llist_t *key_entries, unsigned id)
{
    while (key_entries)
    {
    ...
    if (opts & OPT_k)
    {
        ...
        while (config_read(parser, tokens, 4, 3, "# \t", PARSE_NORMAL | PARSE_MIN_DIE) == 3)
        {
            ...
            if ((tokens[1][0] | 0x20) == 'm')
                ...
                hash_type = HASH_MD5;
            else if (strncasecmp(tokens[1], "sha", 3) == 0)
...
static void set_freq(double freq) /* frequency update */
{
    char tbuf[80];
