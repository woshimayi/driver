...
static char *escape_text(const char *prepend, const char *str, unsigned escapee)
{
...
static unsigned replace_char(char *str, char from, char to)
{
    char *p = str;
    while (*p)
    {
...
static void verbose_log(const char *str)
{
    bb_error_msg("%.*s", (int)strcspn(str, "\r\n"), str);
}

/* NB: status_str is char[4] packed into uint32_t */
static void cmdio_write(uint32_t status_str, const char *str)
{
    char *response;
    int len;
...
static void cmdio_write_ok(unsigned status)
{
...
static void cmdio_write_error(unsigned status)
{
...
static void cmdio_write_raw(const char *p_text)
{
...
static void timeout_handler(int sig UNUSED_PARAM)
{
    off_t pos;
    int sv_errno = errno;
...
214 Help OK.
*/
static void handle_feat(unsigned status)
{
...
static inline int port_active(void)
{
    return (G.port_addr != NULL);
}

static inline int pasv_active(void)
{
    return (G.pasv_listen_fd > STDOUT_FILENO);
}

static void port_pasv_cleanup(void)
{
    free(G.port_addr);
    G.port_addr = NULL;
...
static int ftpdataio_get_pasv_fd(void)
{
    int remote_fd;
 ...
 */
static int get_remote_transfer_fd(const char *p_status_msg)
{
    int remote_fd;
...
static int port_or_pasv_was_seen(void)
{
...
/* Exits on error */
static unsigned bind_for_passive_mode(void)
{
    int fd;
    unsigned port;
...
static int popen_ls(const char *opt)
{
    const char *argv[5];
...
static void handle_dir_common(int opts)
{
    FILE *ls_fp;
    char *line;
    int ls_fd;
...
static void handle_stat_file(void)
{
 ...
 */
static void handle_size_or_mdtm(int need_size)
{
    struct stat statbuf;
...
static void handle_upload_common(int is_append, int is_unique)
{
    struct stat statbuf;
    char *tempname;
    off_t bytes_transferred;
...
static uint32_t cmdio_get_cmd_and_arg(void)
{
    int len;
    uint32_t cmdval;
    char *cmd;
