...
static ssize_t bc_num_cmp(BcNum *a, BcNum *b)
{
    ...
    if (a == b)
        return 0;
    if (a->len == 0)
        return BC_NUM_NEG(!!b->len, !b->neg);
    if (b->len == 0)
        return BC_NUM_NEG(1, a->neg);
    ...
    if (cmp != 0)
        return BC_NUM_NEG(cmp, neg);

    for (max_num -= diff, i = diff - 1; i < diff; --i)
    {
        if (max_num[i])
            return BC_NUM_NEG(1, neg);
    }
...
static FAST_FUNC BC_STATUS zbc_num_d(BcNum *a, BcNum *b, BcNum *restrict c, size_t scale)
{
    ...
    if (b->rdx > cp.rdx)
        bc_num_extend(&cp, b->rdx - cp.rdx);
    cp.rdx -= b->rdx;
    if (scale > cp.rdx)
        bc_num_extend(&cp, scale - cp.rdx);
