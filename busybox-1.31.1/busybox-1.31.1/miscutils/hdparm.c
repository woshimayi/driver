...
static void identify(uint16_t *val)
{
       ...
       )
    {
        ...
        if (jj || kk)
        {
            bb_putchar('\t');
            if (jj)
                printf("%umin for %sSECURITY ERASE UNIT. ", jj == ERASE_BITS ? 508 : jj << 1, "");
            if (kk)
                printf("%umin for %sSECURITY ERASE UNIT. ", kk == ERASE_BITS ? 508 : kk << 1, "ENHANCED ");
            bb_putchar('\n');
        }
    }
...
int hdparm_main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, hdparm_options)) >= 0)
    {
        ...
        do_flush |= (c == 'f');
        if (c == 'u')
            getset_unmask    = parse_opts_0_1(&unmask);
        IF_FEATURE_HDPARM_HDIO_GETSET_DMA(
            if (c == 'd') getset_dma       = parse_opts_0_max(&dma, 9);
        )
            if (c == 'n')
                getset_nowerr    = parse_opts_0_1(&nowerr);
        parse_xfermode((c == 'p'), &noisy_piomode, &set_piomode, &piomode);
        if (c == 'r')
            getset_readonly  = parse_opts_0_1(&readonly);
        if (c == 'm')
            getset_mult      = parse_opts_0_INTMAX(&mult /*32*/);
        if (c == 'c')
            getset_io32bit   = parse_opts_0_INTMAX(&io32bit /*8*/);
        if (c == 'k')
            getset_keep      = parse_opts_0_1(&keep);
        if (c == 'a')
            getset_readahead = parse_opts_0_INTMAX(&Xreadahead);
        if (c == 'B')
            getset_apmmode   = parse_opts(&apmmode, 1, 255);
        do_flush |= do_timings |= (c == 't');
        do_flush |= do_ctimings |= (c == 'T');
#ifdef HDIO_DRIVE_CMD
        if (c == 'S')
            getset_standby  = parse_opts_0_max(&standby_requested, 255);
        if (c == 'D')
            getset_defects  = parse_opts_0_INTMAX(&defects);
        if (c == 'P')
            getset_prefetch = parse_opts_0_INTMAX(&prefetch);
        parse_xfermode((c == 'X'), &get_xfermode, &set_xfermode, &xfermode_requested);
        if (c == 'K')
            getset_dkeep     = parse_opts_0_1(&prefetch);
        if (c == 'A')
            getset_lookahead = parse_opts_0_1(&lookahead);
        if (c == 'L')
            getset_doorlock  = parse_opts_0_1(&doorlock);
        if (c == 'W')
            getset_wcache    = parse_opts_0_1(&wcache);
        get_powermode |= (c == 'C');
        set_standbynow |= (c == 'y');
        set_sleepnow |= (c == 'Y');
        reread_partn |= (c == 'z');
