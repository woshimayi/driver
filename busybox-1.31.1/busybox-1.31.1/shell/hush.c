...
static void reinit_unicode_for_hush(void)
{
       ...
       )
    {
        ...
        if (!s)
            s = get_local_var_value("LC_CTYPE");
        if (!s)
            s = get_local_var_value("LANG");
        reinit_unicode(s);
    }
}
...
static char *encode_then_expand_string(const char *str)
{
    ...
    {
        if (!*cp)
            return NULL; /* string has no special chars */
        if (*cp == '$')
            break;
        if (*cp == '\\')
            break;
...
only_assignments:
        ...
        else if (ENABLE_FEATURE_SH_NOFORK && NUM_APPLETS > 1)
        {
