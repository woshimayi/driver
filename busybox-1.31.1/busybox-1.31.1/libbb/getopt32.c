...
static uint32_t vgetopt32(char **argv, const char *applet_opts, const char *applet_long_options, va_list p)
{
    int argc;
    unsigned flags = 0;
    unsigned requires = 0;
...
#if ENABLE_LONG_OPTS
uint32_t FAST_FUNC getopt32long(char **argv, const char *applet_opts, const char *longopts, ...)
{
    uint32_t opt;
    va_list p;
