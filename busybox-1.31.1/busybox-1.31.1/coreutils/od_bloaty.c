...
static unsigned lcm(unsigned u, unsigned v)
{
    unsigned t = gcd(u, v);
    if (t == 0)
        return 0;
...
static void print_s_char(size_t n_bytes, const char *block, const char *fmt_string)
{
    while (n_bytes--)
    {
...
static void print_char(size_t n_bytes, const char *block, const char *fmt_string)
{
    while (n_bytes--)
    {
...
static void print_s_short(size_t n_bytes, const char *block, const char *fmt_string)
{
...
static void print_short(size_t n_bytes, const char *block, const char *fmt_string)
{
...
static void print_int(size_t n_bytes, const char *block, const char *fmt_string)
{
...
#else
static void print_long(size_t n_bytes, const char *block, const char *fmt_string)
{
...
#else
static void print_long_long(size_t n_bytes, const char *block, const char *fmt_string)
{
...
static void print_float(size_t n_bytes, const char *block, const char *fmt_string)
{
...
static void print_double(size_t n_bytes, const char *block, const char *fmt_string)
{
...
static void print_long_double(size_t n_bytes, const char *block, const char *fmt_string)
{
...
static void print_named_ascii(size_t n_bytes, const char *block,
...
static void print_ascii(size_t n_bytes, const char *block,
                        const char *unused_fmt_string UNUSED_PARAM)
{
    // buf[N] pos:  01234 56789
    char buf[12] = "   x\0 xxx\0";
...
static NOINLINE const char *decode_one_format(const char *s_orig, const char *s, struct tspec *tspec)
{
...
static void format_address_none(off_t address UNUSED_PARAM, char c UNUSED_PARAM)
{
}

static void format_address_std(off_t address, char c)
{
    /* Corresponds to 'c' */
...
static void format_address_paren(off_t address, char c)
{
    putchar('(');
...
static void format_address_label(off_t address, char c)
{
    format_address_std(address, ' ');
...
static void dump_hexl_mode_trailer(size_t n_bytes, const char *block)
{
    fputs("  >", stdout);
    while (n_bytes--)
    {
...
static void write_block(off_t current_offset, size_t n_bytes,
...
static void read_block(size_t n, char *block, size_t *n_bytes_in_buffer)
{
...
static void dump(off_t current_offset, off_t end_offset)
{
    char *block[2];
    int idx;
    size_t n_bytes_read;
...
static void dump_strings(off_t address, off_t end_offset)
{
...
static int parse_old_offset(const char *s, off_t *offset)
{
...
int od_main(int argc UNUSED_PARAM, char **argv)
{
    ...
    if (opt & OPT_A)
    {
        ...
        pos = p - doxn;
        if (pos == 3)
            G.format_address = format_address_none;
    ...
    if (opt & OPT_j)
        n_bytes_to_skip = xstrtooff_sfx(str_j, 0, bkm_suffixes);
    if (opt & OPT_l)
