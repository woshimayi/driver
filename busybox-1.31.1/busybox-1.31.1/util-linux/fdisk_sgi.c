...
static void setfreelist(int i, unsigned int f, unsigned int l)
{
    freelist[i].first = f;
    freelist[i].last = l;
}

static void add2freelist(unsigned int f, unsigned int l)
{
    int i;
    for (i = 0; i < 17; i++)
...
static unsigned int isinfreelist(unsigned int b)
{
    int i;
...
static int sgi_get_nsect(void)
{
    return SGI_SSWAP16(sgilabel->devparam.nsect);
}

static int sgi_get_ntrks(void)
{
    return SGI_SSWAP16(sgilabel->devparam.ntrks);
}

static unsigned int two_s_complement_32bit_sum(unsigned int *base, int size /* in bytes */)
{
    int i = 0;
    unsigned int sum = 0;
...
static int check_sgi_label(void)
{
...
static unsigned int sgi_get_start_sector(int i)
{
...
static int sgi_get_bootpartition(void)
{
    return SGI_SSWAP16(sgilabel->boot_part);
}

static int sgi_get_swappartition(void)
{
...
static unsigned int sgi_get_lastblock(void)
{
...
static int sgi_check_bootfile(const char *aFile)
{
    if (strlen(aFile) < 3) /* "/a\n" is minimum */
    {
        printf("\nInvalid Bootfile!\n"
...
static int compare_start(int *x, int *y)
{
    /*
...
static int verify_sgi(int verbose)
{
    ...
    for (i = 1, start = 0; i < sortcount; i++)
    {
        ...
        if (sgi_get_sysid(Index[i]) == SGI_ENTIRE_DISK)
            continue;
...
static void sgi_set_partition(int i, unsigned int start, unsigned int length, int sys)
{
    sgilabel->partitions[i].id = SGI_SSWAP32(sys);
...
static void sgi_set_entire(void)
{
    int n;
...
static void sgi_set_volhdr(void)
{
    int n;
...
static void create_sgilabel(void)
{
