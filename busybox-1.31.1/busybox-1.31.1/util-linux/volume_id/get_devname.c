 ...
 * NB: closes fd. */
static int get_label_uuid(int fd, char **label, char **uuid, const char **type)
{
    int rv = 1;
    uint64_t size;
...
static void uuidcache_addentry(char *device, /*int major, int minor,*/ char *label, char *uuid, const char *type)
{
...
static int FAST_FUNC uuidcache_check_device(const char *device,
        struct stat *statbuf,
...
static struct uuidCache_s *uuidcache_init(int scan_devices)
{
...
#ifdef UNUSED
static char *get_spec_by_x(int n, const char *t, int *majorPtr, int *minorPtr)
{
...
static char *get_spec_by_uuid(const char *s, int *major, int *minor)
{
...
static char *get_spec_by_volume_label(const char *s, int *major, int *minor)
{
