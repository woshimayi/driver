...
static void set_sun_partition(int i, unsigned start, unsigned stop, int sysid)
{
...
static int check_sun_label(void)
{
    unsigned short *ush;
    int csum;
...
static const struct sun_predefined_drives *sun_autoconfigure_scsi(void)
{
...
static void create_sunlabel(void)
{
    ...
    if (!p || floppy)
    {
        ...
        else
        {
            ...
            sunlabel->pcylcount = SUN_SSWAP16(read_int(0, g_cylinders + SUN_SSWAP16(sunlabel->nacyl), 65535, 0,
                                              "Physical cylinders"));
...
static void fetch_sun(unsigned *starts, unsigned *lens, unsigned *start, unsigned *stop)
{
    int i, continuous = 1;
