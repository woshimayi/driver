...
found_total_clust:
    ...
    if (opts & OPT_c)
    {
        ...
        {
            if (currently_testing + try
                        > volume_size_blocks)
                    try
                            = volume_size_blocks - currently_testing;
            ...
            {
                int cluster = (currently_testing * SECTORS_PER_BLOCK + i - start_data_sector) / (int)(sect_per_clust) /
                              (bytes_per_sect / SECTOR_SIZE);
                if (cluster < 0)
