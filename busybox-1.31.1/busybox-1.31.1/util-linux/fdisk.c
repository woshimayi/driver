...
static const char *partname(const char *dev, int pno, int lth)
{
    const char *p;
    int w, wp;
    int bufsiz;
...
static ALWAYS_INLINE struct partition *get_part_table(int i)
{
    return ptes[i].part_table;
}

static const char *str_units(int n)
{
    /* n==1: use singular */
    if (n == 1)
...
#if ENABLE_FEATURE_FDISK_WRITABLE
static void set_all_unchanged(void)
{
    int i;
...
static ALWAYS_INLINE void set_changed(int i)
{
    ptes[i].changed = 1;
}

static ALWAYS_INLINE void write_part_table_flag(char *b)
{
    b[510] = 0x55;
    b[511] = 0xaa;
}
...
static char read_nonempty(const char *mesg)
{
...
static char read_maybe_empty(const char *mesg)
{
    if (!read_line(mesg))
    {
        line_ptr = line_buffer;
...
static int read_hex(const char *const *sys)
{
    unsigned long v;
    while (1)
    {
...
static void write_sector(sector_t secno, const void *buf)
{
    seek_sector(secno);
...
static uint16_t fdisk_swap16(uint16_t x)
{
    return (x << 8) | (x >> 8);
}

static uint32_t fdisk_swap32(uint32_t x)
{
    return (x << 24) |
           ((x & 0xFF00) << 8) |
           ((x & 0xFF0000) >> 8) |
...
static inline_if_little_endian unsigned read4_little_endian(const unsigned char *cp)
{
    uint32_t v;
...
static inline_if_little_endian void store4_little_endian(unsigned char *cp, unsigned val)
{
...
#if ENABLE_FEATURE_FDISK_WRITABLE
static const char *const *get_sys_types(void)
{
    return (
...
#if ENABLE_FEATURE_FDISK_WRITABLE
static void warn_cylinders(void)
{
...
#if ENABLE_FEATURE_FDISK_WRITABLE
static void create_doslabel(void)
{
...
static int get_existing_partition(int warn, unsigned max)
{
    int pno = -1;
    unsigned i;
...
static void toggle_active(int i)
{
    struct pte *pe = &ptes[i];
    struct partition *p = pe->part_table;
...
static void toggle_dos_compatibility_flag(void)
{
...
static void fix_partition_table_order(void)
{
    struct pte *pei, *pek;
    int i, k;
...
#if ENABLE_FEATURE_FDISK_ADVANCED
static void x_list_table(int extend)
{
    const struct pte *pe;
    const struct partition *p;
    int i;
...
static void print_buffer(char *pbuffer)
{
    int i, l;
...
static void move_begin(unsigned i)
{
    struct pte *pe = &ptes[i];
    struct partition *p = pe->part_table;
...
static int is_ide_cdrom_or_tape(const char *device)
{
    FILE *procf;
    char buf[100];
    struct stat statbuf;
...
static void open_list_and_close(const char *device, int user_specified)
{
    int gb;
...
static void list_devs_in_proc_partititons(void)
{
    FILE *procpt;
...
#if ENABLE_FEATURE_FDISK_WRITABLE
static void unknown_command(int c)
{
