...
static void check_blocks(void)
{
    ...
    while (G.currently_testing < SB_ZONES)
    {
        ...
        try
                = TEST_BUFFER_BLOCKS;
        if (G.currently_testing + try
                    > SB_ZONES)
                try
                        = SB_ZONES - G.currently_testing;
...
int mkfs_minix_main(int argc UNUSED_PARAM, char **argv)
{
    ...
    //if (opt & 1) -c
    if (opt & 2)
        G.req_nr_inodes = xatoul(str_i); // -i
    //if (opt & 4) -l
    if (opt & 8)   // -n
    {
        if (G.namelen == 14)
            G.magic = MINIX1_SUPER_MAGIC;
        else if (G.namelen == 30)
            G.magic = MINIX1_SUPER_MAGIC2;
        else
            bb_show_usage();
