...
static struct obj_symbol *obj_add_symbol(struct obj_file *f, const char *name,
...
static struct obj_symbol *obj_find_symbol(struct obj_file *f, const char *name)
{
    struct obj_symbol *sym;
    unsigned long hash = f->symbol_hash(name) % HASH_BUCKETS;
...
static void obj_insert_section_load_order(struct obj_file *f, struct obj_section *sec)
{
...
static void new_process_module_arguments(struct obj_file *f, const char *options)
{
    char *xoptions, *pos;
    char *param, *val;
...
static int new_get_module_version(struct obj_file *f, char str[STRVERSIONLEN])
{
    char *p, *q;
    int a, b, c;
...
static int new_init_module(const char *m_name, struct obj_file *f, unsigned long m_size)
{
