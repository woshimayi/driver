/*
 * @*************************************:
 * @FilePath     : /network/Netlink/netlink_monitor_ifname_status/netlink_monitor_ifanme_status.c
 * @version      :
 * @Author       : dof
 * @Date         : 2024-08-09 17:38:19
 * @LastEditors  : dof
 * @LastEditTime : 2024-08-09 18:08:10
 * @Descripttion : 监控网络接口down up ipaddrss 状态
 * @compile      :
 * @**************************************:
 */
...
#define nl_log(fmt, args...) printf("\033[0;32;31m[netlink :%s(%d)] " fmt "\033[1;37m\r\n", __func__, __LINE__, ##args)
...
void parseBinaryNetlinkMessage(struct nlmsghdr *nh)
{
    ...
    {
        nl_log("[netlink] Got a short RTM_NEWLINK message\n");
        return;
    }
...
void parseNetlinkAddrMsg(struct nlmsghdr *nlh, int new)
{
    ...
    while (rtl && RTA_OK(rth, rtl))
    {
        if (rth->rta_type == IFA_LOCAL)
        {
            ...
            nl_log("[netlink] %s %s address %d.%d.%d.%d\n",
                   name, (new != 0) ? "add" : "del",
                   (ipaddr >> 24) & 0xff,
                   (ipaddr >> 16) & 0xff,
                   (ipaddr >> 8) & 0xff,
