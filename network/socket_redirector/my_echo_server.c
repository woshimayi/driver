...
#undef  printf
#define printf(format,...) fprintf(terminal_log_file, format, ##__VA_ARGS__)
...
int main()
{
    ...
    if (terminal_log_file == NULL)
    {
        ...
        fprintf(
            stderr, "Echo Server: Warning: Could not open /dev/tty for logging. Logs might go to client or nowhere.\n");
    ...
    printf("Echo Server: Started. Reading from stdin, writing to stdout. (Logs to terminal via /dev/tty)\n");
    ...
    while (fgets(buffer, sizeof(buffer), stdin) != NULL)
    {
        ...
        printf("Echo Server: Received from client: %s", buffer); // 调试信息
        fflush(terminal_log_file);

        // 向标准输出写入数据。
        // printf 会将数据写入 stdout。
        // 因为 stdout 被 dup2 重定向到套接字，printf 就会将数据发送给客户端
        fprintf(stderr, "Echo: %s", buffer); // 直接打印接收到的字符串，包括其可能带的换行符
