...
int socket_select(int argc, char *argv[])
{
    char buf[BUFSIZE];
    size_t nbytes;
    int sockfd, maxfd1;
...
int main(int argc, char const *argv[])
{
    ...
    while ((c = getopt(argc, argv, "vhs:N:I:p::")) != -1)
    {
        switch (c)
        {
            case 'v':
            ...
            case 's':
                printf("sss %s %d\n", optarg, optind);
                ntpconf.server[i++] = strdup(optarg);
                // strncpy(ntpconf.server[i++], optarg, 128);
                break;
            ...
            case 'N':
                ...
                {
                    printf("zzz %s %d\n", optarg, optind);
                    ntpconf.deltaTime = atoi(optarg);
                }
            break;

            case 'I':
                strncpy(ntpconf.ifname, optarg, sizeof(ntpconf.ifname));
                break;

            case 'p':
                if (NULL != optarg)
                {
                    ntpconf.protol = atoi(optarg);
                }
                break;
    ...
    printf(" %s %s %s %d  %d\n", ntpconf.server[0], ntpconf.server[1], ntpconf.ifname, ntpconf.protol, ntpconf.deltaTime);
