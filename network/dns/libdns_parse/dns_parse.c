                            ...
                            int timeout_sec)
{
    ...
    if (domain == NULL || (query_type != 4 && query_type != 6))
    {
    ...
    bool ipv6_server = false;

    // If dns_server is NULL, use a default DNS server (e.g., 8.8.8.8 or 2001:4860:4860::8888)
    const char* default_dns_server_ipv4 = "223.5.5.5";
    const char* default_dns_server_ipv6 = "2400:3200::1";
    const char* effective_dns_server = dns_server;

    if (dns_server == NULL)
    {
        ipv6_server = ipv6_query; // Use IPv6 default if it's an IPv6 query
        effective_dns_server = ipv6_query ? default_dns_server_ipv6 : default_dns_server_ipv4;
    }
    else
    {
        ipv6_server = (strchr(dns_server, ':') != NULL);
    }
    ...
    // Only bind to the interface if it's provided
    if (interface != NULL && bind_to_interface(sockfd, interface) != 0)
    {
        snprintf(result.error_message, sizeof(result.error_message), "Failed to bind to interface: %s",
                 strerror(errno));
        close(sockfd);
        result.error_code = -3;
        return result;
    ...
    if (ipv6_server)
    {
        ...
        if (inet_pton(AF_INET6, effective_dns_server, &servaddr6->sin6_addr) <= 0)
        {
    ...
    else
    {
        ...
        if (inet_pton(AF_INET, effective_dns_server, &servaddr4->sin_addr) <= 0)
        {
    ...
    if (recv_len < 0)
    {
        ...
        {
            snprintf(result.error_message, sizeof(result.error_message), "Failed to receive response: %s",
                     strerror(errno));
