...
// 创建DNS查询报文
int create_dns_query(const char *domain, unsigned char *buffer, int buffer_size, bool ipv6_query)
{
    ...
    {
        query[pos++] = 0;
        query[pos++] = 28; // QTYPE=AAAA (IPv6地址)
    }
    else
    {
        query[pos++] = 0;
        query[pos++] = 1; // QTYPE=A (IPv4地址)
    }
    query[pos++] = 0;
...
void parse_dns_response(unsigned char *buffer, int len)
{
    ...
    for (i = 0; i < ntohs(header->ancount); i++)
    {
        ...
        { // AAAA记录
            ...
            inet_ntop(AF_INET6, &addr6, ip6_str, INET6_ADDRSTRLEN);
            printf("  AAAA Record: %s\n", ip6_str);
        }
        else if (type == 5)
        { // CNAME记录
            printf("  CNAME Record: ");
...
int main(int argc, char *argv[])
{
    if (argc < 4)
    {
        fprintf(stderr, "Usage: %s <domain> <dns_server> <query_type> [interface]\n", argv[0]);
        fprintf(stderr, "  query_type: 4 for IPv4 (A record), 6 for IPv6 (AAAA record)\n");
        fprintf(stderr, "Example: %s example.com 8.8.8.8 4 eth0\n", argv[0]);
        fprintf(stderr, "Example: %s example.com 2001:4860:4860::8888 6 eth0\n", argv[0]);
        return 1;
    }
    ...
    const char *interface = argc > 4 ? argv[4] : NULL;

    if (query_type != 4 && query_type != 6)
    {
        fprintf(stderr, "Invalid query type. Use 4 for IPv4 or 6 for IPv6\n");
        return 1;
    }
    ...
    if (ipv6_server)
    {
        ...
        {
            perror("invalid IPv6 DNS server address");
            close(sockfd);
            return 1;
        }
    ...
    else
    {
        ...
        {
            perror("invalid IPv4 DNS server address");
            close(sockfd);
            return 1;
        }
    ...
    // 发送DNS查询
    printf("Sending DNS %s query for %s to %s\n",
           ipv6_query ? "AAAA (IPv6)" : "A (IPv4)", domain, dns_server);
    if (interface)
    {
