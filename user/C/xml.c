...
static void recoverFactoryParam(UBOOL8 isRecover)
{
    ...
    if (SF_FEATURE_SUPPORT_WLAN)
    {
        if(SF_FEATURE_CUSTOMER_JINQIANMAO || SF_FEATURE_CUSTOMER_DONGYAN || SF_FEATURE_CUSTOMER_JSWX
         || (SF_FEATURE_CUSTOMER_KINGTYPE && SF_FEATURE_LOCATION_SHAANXI) || SF_FEATURE_CUSTOMER_H3C || SF_FEATURE_CUSTOMER_JISHIHUITONG)
        {
            if (restoreMode)
            {
                UINT8 ucTemp[6] = {0};
        ...
        if (wlanSsid[0] != '\0')
        {
            ...
            if (mode != 1)
            {
                if (wlanSsid2[0] != '\0')
                {
                    ...
                    {
                        if ((1 == wirelessObj->enable) || SF_FEATURE_LOCATION_HUBEI || SF_FEATURE_CUSTOMER_DONGYAN || SF_FEATURE_LOCATION_HUNAN || SF_FEATURE_LOCATION_SHAANXI
                        || (SF_FEATURE_LOCATION_BEIJING && SF_FEATURE_ISP_CU) || ((SF_FEATURE_LOCATION_SHANGHAI || SF_FEATURE_LOCATION_HAINAN) && SF_FEATURE_ISP_CU)
                ...
                if (wlanSsid3[0] != '\0')
                {
                    ...
                    {
                        setSSID3Flag = TRUE;
                        if (1 == wirelessObj->enable || SF_FEATURE_LOCATION_HUBEI || ((SF_FEATURE_LOCATION_BEIJING || SF_FEATURE_LOCATION_SHANGHAI || SF_FEATURE_LOCATION_HAINAN) && SF_FEATURE_ISP_CU)
                        || (SF_FEATURE_LOCATION_SICHUAN && SF_FEATURE_CUSTOMER_JINQIANMAO))
                        {
        ...
        if (buf[0] != '\0')
        {
            ...
            {
                vosLog_error("get MDMOID_LAN_WLAN_PRE_SHARED_KEY(%d:%s) error", MDMOID_LAN_WLAN_PRE_SHARED_KEY, mdm_dumpIidStack(&iidStack));
                return;
            }
