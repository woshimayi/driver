...
/*
	setitimer() 还可以根据进程的不同运行状态来控制定时功能的运行状态。setitimer() 是一个可以在本轮定时任务完成后自动重启下一轮定时的定时机制，当然是否自动重启取决于启动定时器时参数2的值。在上面函数参数2的释义中，struct itimerval 中的 it_value 成员表示定时器当前的值，它可以设置一个秒值及微秒值，其实就是你想让这个定时器在多少时间以后启动，立即启动则将 it_value 的值全设成0。而 it_interval 则表示你想定时多少时间。假如我们给 it_value 设置了一个值，但 it_interval 却全设成0，就表示定时器将在 it_value 设置的时间以后发出一个定时到时信号，此后就不会再自动重启下一轮定时任务了
*/
