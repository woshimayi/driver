/*
 * @*************************************: 
 * @FilePath: /user/C/string/calc_crc32.c
 * @version: 
 * @Author: dof
 * @Date: 2021-10-20 19:33:39
 * @LastEditors: dof
 * @LastEditTime: 2021-12-07 14:11:08
 * @Descripttion: crc32 文件校验
 * @**************************************: 
 */
#include <stdio.h>
#include <string.h>
...
#define IMAGE_LOCALPATH "image.bin"
...
int image_calc_crc32(const char * file)
{
    ...
    fread(value, sizeof(char), 512, pfin);
    fseek(pfin, 0, SEEK_END);
    filesize = ftell(pfin);
    rewind(pfin);

    buff = (unsigned char *)malloc(filesize);
    readlen = fread(buff, 1, filesize - 4, pfin);
...
void calc_len(const char * file)
{
    ...
    snprintf(value, sizeof(value), "%02X%02X%02X%02X", (writeCrc >> 24) & 0xFF, (writeCrc >> 16) & 0xFF,
             (writeCrc >> 8)  & 0xFF, (writeCrc >> 0)  & 0xFF);
    DBG_MSG("222 value = %s\n\n\n", value);
    fclose(fid);
}
