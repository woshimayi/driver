...
#define cpp_overload(...)                       \
	CAT(cpp_overload, COUNT_PARMS(__VA_ARGS__)) \
	(__VA_ARGS__)
...
#define cpp_default(...)                       \
	CAT(cpp_default, COUNT_PARMS(__VA_ARGS__)) \
	(__VA_ARGS__)
...
#define sizeof_overload(A)                                                                                                      \
	((sizeof(A) == sizeof(float)) ? sizeof_overload_float(A) : (sizeof(A) == sizeof(double))    ? sizeof_overload_double(A)     \
...
#define gcc_type_overload(A) \
	gcc_type_overload_aux(   \
	                         __builtin_types_compatible_p(typeof(A), struct s1) * 1 + __builtin_types_compatible_p(typeof(A), struct s2) * 2, A)
...
#define TYPENUM(A) \
	__builtin_types_compatible_p(typeof(A), int) * 1 + __builtin_types_compatible_p(typeof(A), long long) * 2 + __builtin_types_compatible_p(typeof(A), double) * 3 + __builtin_types_compatible_p(typeof(A), long double) * 4
...
#define param_lister(...)                       \
	CAT(param_lister, COUNT_PARMS(__VA_ARGS__)) \
	(__VA_ARGS__)
