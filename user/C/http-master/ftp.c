...
void ftp_connect(struct url *url, struct url *proxy, int timeout)
{
	char		*buf = NULL;
	size_t		 n = 0;
	int		 sock;
...
void ftp_save(struct url *url, FILE *dst_fp, off_t *offset)
{
...
int ftp_getline(char **lineptr, size_t *n, int suppress_output, FILE *fp)
{
	ssize_t		 len;
	char		*bufp, code[4];
	const char	*errstr;
...
int ftp_command(FILE *fp, const char *fmt, ...)
{
	va_list	 ap;
	char	*buf = NULL, *cmd;
	size_t	 n = 0;
...
int ftp_auth(FILE *fp, const char *user, const char *pass)
{
...
int ftp_size(FILE *fp, const char *fn, off_t *sizep, char **buf)
{
	size_t	 n = 0;
	off_t	 file_sz;
	int	 code;
