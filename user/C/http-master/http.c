...
void http_connect(struct url *url, struct url *proxy, int timeout)
{
...
void http_save(struct url *url, FILE *dst_fp, off_t *offset)
{
    if (url->chunked)
...
static struct url *http_redirect(struct url *old_url, char *location)
{
    struct url	*new_url;
...
static char *relative_path_resolve(const char *base_path, const char *location)
{
    char	*new_path, *p;
...
static void http_save_chunks(struct url *url, FILE *dst_fp, off_t *offset)
{
    char	*buf = NULL;
    size_t	 n = 0;
    uint	 chunk_sz;
...
static void decode_chunk(int scheme, uint sz, FILE *dst_fp)
{
    size_t	bufsz;
    size_t	r;
...
void http_close(struct url *url)
{
#ifndef NOSSL
    ssize_t	r;
...
static int http_request(int scheme, const char *req, struct http_headers **hdrs)
{
    struct http_headers	*headers;
    const char		*e;
    char			*buf = NULL, *p;
...
static void http_headers_free(struct http_headers *headers)
{
    if (headers == NULL)
        return;
...
static char *header_lookup(const char *buf, const char *key)
{
    char	*p;
...
static int http_status_cmp(const void *a, const void *b)
{
...
static ssize_t http_getline(int scheme, char **buf, size_t *n)
{
    ssize_t	buflen;
...
static size_t http_read(int scheme, char *buf, size_t size)
{
    size_t	r;
#ifndef NOSSL
    ssize_t	rs;
...
#ifndef NOSSL
void https_init(char *tls_options)
{
    char		*str;
    int		 depth;
...
static ssize_t tls_getline(char **buf, size_t *buflen, struct tls *tls)
{
    char		*newb;
    size_t		 newlen, off;
    int		 ret;
...
static void tls_copy_file(struct url *url, FILE *dst_fp, off_t *offset)
{
    char	*tmp_buf;
    ssize_t	 r;
