...
UBOOL8 matchChipId(const char *strTagChipId, const char *signature2 __attribute__((unused)))
{
    ...
    {
        hgslog_write_item(HLOG_RW_UPGRADE_VER_STATUS, 1);
...
CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, UINT32 opts, int * noReboot)
{
    ...
    if (cfeAddr)
    {
        ...
        if (ret != CMSRET_SUCCESS)
        {
            hgslog_write_item(HLOG_RW_UPGRADE_VER_STATUS, 4);
    ...
    if (rootfsAddr && kernelAddr)
    {
        ...
        hgslog_write_item(HLOG_RW_UPGRADE_VER_STATUS, 3);
    ...
    hgslog_write_item(HLOG_RW_UPGRADE_VER_STATUS, 5);
...
CmsRet cmsImg_writeImage(char *imagePtr, UINT32 imageLen, void *msgHandle)
{
   ...
   {
      //rename(FILE_NEW_SOFTWARE_VER, FILE_OLD_SOFTWARE_VER);
      rename("/data/new_soft_ver", "/data/old_soft_ver");
      hgslog_write_item(HLOG_RW_UPGRADE_VER_STATUS, 2);
