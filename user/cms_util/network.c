...
CmsRet cmsNet_getMacAddrByIfname(const char *ifname, unsigned char *macAddr)
{
   ...
   {
      cmsLog_debug("SIOCGIFHWADDR returns error.");
...
{
    return cmsUtl_genEui64AddrByPrefix(prefix, ifName, ulAddr, prefixLen);
}

#endif  /* SUPPORT_IPV6 */


void cmsUlt_getSubNet(char*ipAddr, char*mask, char*subNet)
{
    
    struct in_addr ipIn, maskIn, subnetIn;

    inet_aton(ipAddr, &ipIn);
    inet_aton(mask, &maskIn);
    subnetIn.s_addr = ipIn.s_addr & maskIn.s_addr;
    sprintf(subNet, "%s", inet_ntoa(subnetIn));
