                  ...
                  CmsTimestamp *deltaTms)
{
   if (newTms->sec >= oldTms->sec)
   {
      ...
      else
      {
		 ...
		 /* modify by caoshaohua, 2022-02-21 */
		 #if 1
		 if (newTms->sec > oldTms->sec)
		 {
		 ...
		 #else
	#if 0 //modified by hlb,2018.07.11
         deltaTms->sec = newTms->sec - oldTms->sec - 1;
	#else
	   deltaTms->sec = newTms->sec - oldTms->sec;
	#endif
...
#if 0 //modified by hlb,2018.07.13
      if ((MAX_UINT32 - ms) < (deltaTms.nsec / NSECS_IN_MSEC))
#else
      if (ms > 0 && ((MAX_UINT32 - ms) < (deltaTms.nsec / NSECS_IN_MSEC)))
#endif
      {
...
//fucheng porting start for sichuan httpdownload 2015/11/23
CmsRet cmsTms_getXSIDateTime0(UINT32 t, char *buf, UINT32 bufLen)
{
   int 		 c;
   time_t		now;
	struct tm	*tmp;
...
//fucheng porting end 2015/11/23
CmsRet cmsTms_getXSIDateTimeByFormat(UINT32 t, const char *buf, UINT32 bufLen, const char *format)
{
   return (oalTms_getXSIDateTimeByFormat(t, (char *)buf, bufLen,(char *)format));
}
