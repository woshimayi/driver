...
static CTMDW_STATUS parseCTMDWParaAttrValues(PCTMDW_TLV tlv, char *twType)
{
    ...
    for (i = 0; i < numOfParam; i++)
    {
        ...
        if ((!strncmp(twType, CTMDW_TW_MDW, util_strlen(CTMDW_TW_MDW)) && strcmp(pValue, CTMDW_ATTRIBUTES_MDW_NOINFORM) &&
                strcmp(pValue, CTMDW_ATTRIBUTES_MDW_INFORM)) ||
                (!strncmp(twType, CTMDW_TW_TR69, util_strlen(CTMDW_TW_TR69)) &&
                 strcmp(pValue, CTMDW_ATTRIBUTES_ITMS_NOINFORM_READABLE) &&
                 strcmp(pValue, CTMDW_ATTRIBUTES_ITMS_REBOOTINFORM_READABLE) && strcmp(pValue, CTMDW_ATTRIBUTES_ITMS_INFORM_READABLE) &&
                 strcmp(pValue, CTMDW_ATTRIBUTES_ITMS_NOINFORM_WRITABLE) &&
                 strcmp(pValue, CTMDW_ATTRIBUTES_ITMS_REBOOTINFORM_WRITABLE) && strcmp(pValue, CTMDW_ATTRIBUTES_ITMS_INFORM_WRITABLE)))
            return ret = CTMDW_STATUS_ERR_PARAM;
...
static CTMDW_STATUS ctmdw_doFileGet(PCTMDW_MSG ctmdw_msg)
{
    ...
    if ((cfgBuf != NULL) && (cmcMgm_writeConfigToBuf(cfgBuf, &cfgBufLen) == VOS_RET_SUCCESS))
    {
        ...
        ctmdw_msg->tvlLst->rvalue = VOS_MALLOC(util_strlen(CTMDW_CONFIG_FWCONFIG) + util_strlen(cfgBufLen) + util_strlen(
                CTMDW_VALUE_DELIM) + 1);
...
void init_ctmdwDefNotification(void)
{
    ...
    ctmdw_doParameterAttrSetSimple("InternetGatewayDevice.DeviceInfo.X_CT-COM_MiddlewareMgt.MiddlewareURL", "1",
                                   atoi(CTMDW_TW_MDW));
